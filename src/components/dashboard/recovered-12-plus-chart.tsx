...
interface Recovered12PlusChartProps {
  selectedDate: Date;
  onDateChange: (date: Date) => void;
}

export function Recovered12PlusChart({ selectedDate, onDateChange }: Recovered12PlusChartProps) {
  const firestore = useFirestore();

  const dataRef = useMemoFirebase(
    () => (firestore ? query(collection(firestore, 'recovered_12_plus'), orderBy('date')) : null),
    [firestore]
  );
  ...
  const handleDownloadCsv = () => {
    ...
    link.setAttribute('download', `reporte-recuperados-12-mas-${monthYear}.csv`);
