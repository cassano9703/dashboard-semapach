...
import { format, parse, isValid, startOfMonth, endOfMonth, getDate, getDaysInMonth } from "date-fns";
...
export function Recovered2to3CRUD() {
  const firestore = useFirestore();
  const { toast } = useToast();
  
  const dataRef = useMemoFirebase(
    () => (firestore ? query(collection(firestore, 'recovered_2_to_3'), orderBy('date', 'desc')) : null),
    [firestore]
  );
  const { data: recoveredData, isLoading, error } = useCollection(dataRef);

  const [date, setDate] = useState<Date>();
  const [dailyAmount, setDailyAmount] = useState('');
  const [editingId, setEditingId] = useState<string | null>(null);
  
  const sortedData = recoveredData || [];

  const recalculateMonthAndCommit = (monthDate: Date) => {
    if (!firestore) return;
    ...
    getDocs(q).then(snapshot => {
      ...
      snapshot.docs.forEach(docSnap => {
        ...
        accumulatedTotal += data.dailyAmount;
        batch.update(doc(firestore, 'recovered_2_to_3', docSnap.id), {
          accumulatedMonthly: accumulatedTotal
        });
      });
  ...
  const handleEdit = (item: any) => {
    ...
    setDailyAmount(item.dailyAmount.toString());
  };
  ...
  const handleDelete = (item: any) => {
    if (!firestore) return;
    const docRef = doc(firestore, "recovered_2_to_3", item.id);
    const itemDate = parse(item.date, 'yyyy-MM-dd', new Date());
  ...
  const handleAddOrUpdate = async () => {
    ...
    try {
        if (editingId) {
            const docRef = doc(firestore, 'recovered_2_to_3', editingId);
            await updateDoc(docRef, {
                dailyAmount: newDailyAmount,
                updatedAt: Timestamp.now(),
            });
        } else {
            const q = query(collection(firestore, 'recovered_2_to_3'), where('date', '==', formattedDate));
            const existingDocs = await getDocs(q);
            if (!existingDocs.empty) {
                toast({ variant: 'destructive', title: 'Error', description: `Ya existe un registro para la fecha ${formattedDate}.`});
                return;
            }
            await addDoc(collection(firestore, 'recovered_2_to_3'), {
                date: formattedDate,
                dailyAmount: newDailyAmount,
                accumulatedMonthly: 0, // Will be recalculated
                updatedAt: Timestamp.now(),
            });
        }
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Recuperados 2 a 3 Meses (No Factibles)</CardTitle>
        <CardDescription>
            Gestionar la recuperación diaria y el acumulado del mes.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 items-end gap-4 p-4 border rounded-lg">
              ...
              <PopoverContent className="w-auto p-0">
                <Calendar
                  ...
                  disabled={(day) => {
                    ...
                    // Disable from day 1 to 15, and potentially day 31
                    return dayOfMonth < 16 || (daysInMonth === 31 && dayOfMonth === 31);
                  }}
                />
              </PopoverContent>
            </Popover>
          ...
          <div className="grid gap-2">
            <Label htmlFor="daily-amount">Recuperación Diaria</Label>
            <Input id="daily-amount" placeholder="S/ 0.00" type="number" value={dailyAmount} onChange={(e) => setDailyAmount(e.target.value)} />
          </div>
            ...
            <Table>
              <TableHeader className="sticky top-0 bg-table-header text-table-header-foreground z-10">
                <TableRow>
                  ...
                  <TableHead>Recuperación Diaria</TableHead>
              ...
              <TableBody>
                {isLoading ? (
                  ...
                  <TableRow key={item.id}>
                    ...
                    <TableCell>{formatCurrency(item.dailyAmount)}</TableCell>
                    <TableCell>{formatCurrency(item.accumulatedMonthly)}</TableCell>
